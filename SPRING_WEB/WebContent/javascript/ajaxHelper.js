//##############################################################################
// ajaxHelper contains the script for enabling ajax style.
//##############################################################################
...
v_css.type="text/css";
v_css.href="./stylesheets/bib.css";
document.getElementByTagName('head')[0].appendChild(v_css);
v_css.href="./stylesheets/newbib.css";
document.getElementByTagName('head')[0].appendChild(v_css);
...
function normal(selectedIndex,responseList){
	...
	var str="";
	var cssClass='normalbgwc';
	if(responseList.length > 1){
		str+='<table BORDER="1" cellspacing="0" cellpadding="2" STYLE="border-color:#3399cc;border-collapse:collapse;width:'+txtBoxWidth+'px;" onmouseover="focusme=true;" onmouseout="focusme=false;">'+"\n";
		for(i=0;i<responseList.length;i++){
			...
			str+='<tr>';
			str+='<td class='+'"'+cssClass+'"'+'padding-left:5px;width:'+txtBoxWidth+'px" onclick="endup(\'m\');autocompletelayer.style.display=\'none\';" onmousemove="normal(\''+i+'\',\''+namestr+'\);">'+namestr+'</td>';
			str+='</tr>'+"\n";
		}
		str+='</table>';
	}
	...
	if(str.length > 0 && str.match(new RegExp("[a-z]+", "i")) && KeyID!=37 && KeyID!=39)
	{
		if(!checkExists(str))
		{
			autocompletelayer.innerHTML="";
				var url="http://localhost:8080/SPRING_WEB/CustomerListViewDisplay.do?showAjax=true?&str="+str;
	...
	if(httpObj.readyState == 4)
	{
		try
		{
			if(httpObj.status == 200)
			{
				...
				{
					autocompletelayer.innerHTML="aaaaa";
//					"../images/cssImages/search-loader.gif"
				}
			}
		}
		catch (err)
